...
if bookmarks:
    ...
    avsp.MsgBox(_('There are no bookmarks'), _('Error'))
    return
...
for i, frame_range in enumerate(frames):
    ...
    for j, frame in enumerate(frame_range):
        ret = self.SaveImage(filename % (frame), 
                             frame=frame, avs_clip=AVS, quality=100, depth=8)
        if not ret:
            break
        paths.append(ret)
else:
